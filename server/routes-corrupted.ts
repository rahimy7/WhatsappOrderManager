...
async function processAutoResponse(trigger: string, phoneNumber: string, storeId?: number | null, phoneNumberId?: string | null, tenantStorage?: any) {
  try {
    // Use tenantStorage if provided, otherwise use global storage
    const storageToUse = tenantStorage || storage;
    const autoResponses = await storageToUse.getAllAutoResponses();
  ...
  // Function to process customer messages and responses
            ...
            if (!targetStoreId) {
              await storage.addWhatsAppLog({
                ...
                status: 'failed',
                rawData: JSON.stringify({ availableStores: 2 }) // Known: 2 active stores
              });
  ...
  async function isOrderMessage(text: string): Promise<boolean> {
    return text.startsWith('🛍️ *NUEVO PEDIDO*');
          ...
          try {
            ...
            // STEP 2.5: Process interactive messages FIRST (before customer lookup)
            if (messageType === 'interactive' && message.interactive?.button_reply?.id) {
              ...
              // Process the button directly using tenant storage
              const { processAutoResponse } = await import('./whatsapp-simple');
              const processed = await processAutoResponse(buttonId, from, targetPhoneNumberId, tenantStorage);
              
              if (processed) {
                await storage.addWhatsAppLog({
                  type: 'info',
                  phoneNumber: from,
                  messageContent: `Respuesta automática enviada para botón: ${buttonId}`,
                  status: 'processed'
                });
                return; // Exit early after processing button
              }
            }
