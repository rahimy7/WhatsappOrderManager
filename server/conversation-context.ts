...
export class ConversationContextService {
  ...
  async saveContext(context: ConversationContext): Promise<ConversationContext> {
    const query = `
      ...
      (phone_number, customer_id, current_flow, context_data, selected_order_id)
      VALUES ($1, $2, $3, $4, $5)
  ...
  async getContext(phoneNumber: string): Promise<ConversationContext | null> {
    ...
    const result = await this.storage.query(query, [phoneNumber]);
    ...
    return {
      ...row,
      contextData: typeof row.context_data === 'string' ? 
        JSON.parse(row.context_data) : row.context_data
    };
  }

  async clearContext(phoneNumber: string): Promise<void> {
    const query = `DELETE FROM conversation_context WHERE phone_number = $1`;
    await this.storage.query(query, [phoneNumber]);
  }

  async cleanupExpiredContexts(): Promise<number> {
    const query = `DELETE FROM conversation_context WHERE expires_at < CURRENT_TIMESTAMP`;
